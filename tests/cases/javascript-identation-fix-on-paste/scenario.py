...
def run(ef_path):
    ...
    source_file = os.path.join(test_dir, "test.js")
    insert_file = os.path.join(test_dir, "js_insert.txt")
    expected_file = os.path.join(test_dir, "js_result.js")

    # Work on a copy so the original seed file is preserved
    work_file = os.path.join(test_dir, "work.js")
    shutil.copy2(source_file, work_file)

    with open(insert_file, "r") as f:
        paste_content = f.read()

    # JS uses spaces (expandtab), no conversion needed
